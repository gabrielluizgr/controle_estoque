...
def listar_produtos_gui(janela_principal): # Criação de uma nova função que será chamada quando o usuário clicar no botão Listar Produtos
    ...
    janela_listagem.geometry("500x400") # Define o tamanho da janela
    ...
    if sucesso:
        produtos = resultado
        if produtos:
            for id, nome, marca, quantidade, preco in produtos:
                preco_formatado = f"{preco:.2f}".replace('.', ',')
                texto = f"ID: {id} | Nome: {nome} | Marca: {marca or 'Sem marca'} | Quantidade: {quantidade} | Preço: R$ {preco_formatado}"
